<x-app-layout>
    ...
    <div class="container">
        <x-slot name="header">
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white flex justify-between">operations</h5>   


        <!-- Filter Form -->
            <form action="{{ route('operations.index') }}" method="GET" class=" w-full p-4 bg-white rounded flex  gap-2">
                @csrf
                <div class="w-3/4">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 w-full">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $operation->id }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $operation->Date }}</td>
                    <td class="px-6 py-4 ">{{ $operation->Objet }}</td>
                    <td class="px-6 py-4 ">{{ $operation->Observation }}</td>
